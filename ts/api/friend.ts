...
import {
  NIMFriendAPI, NIMverify_type, NIMDeleteFriendOption, NIMFriendProfile, NIMFriendOptCallback,
  NIMGetFriendsListCallback, NIMGetFriendProfileCallback, NIMFriendChangeCallback, NIMFriendChangeEvent,
} from '../def/friend_def';
...
export class NIMFriend extends ev.EventEmitter {
  ...
  initEventHandler(): void {
    /** (全局回调)好友变更通知回调函数（多端同步添加、删除、更新，好友列表同步）
        * @param cb	好友变更通知回调函数
        ...
        * </pre>
        */
    this.friend.RegChangeCb((result: NIMFriendChangeEvent) => {
