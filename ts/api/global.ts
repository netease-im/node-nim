...
import {
  NIMGlobalAPI, NIMProxyType, NIMCachedFileType, NIMExceptionCallback, NIMDetectProxyCallback, NIMGetCachedFileInfoCallback,
  NIMDeleteCachedFileCallback, NIMSDKFeedbackCallback, NIMSDKDBErrorCallback, NIMUploadSDKLogCallback, NINSDKDBErrorInfo,
} from '../def/global_def';
...
export class NIMGlobal extends ev.EventEmitter {
  ...
  /** 获取sdk缓存文件信息
     ...
     * @return void 无返回值
     */
  getSDKCachedFileInfoAsync(loginId: string, fileType: NIMCachedFileType, endTimestamp: number, cb: NIMGetCachedFileInfoCallback,
  ...
  /** 删除sdk缓存文件
     ...
     * @return void 无返回值
     */
  deleteSDKCachedFileAsync(loginId: string, fileType: NIMCachedFileType, endTimestamp: number, cb: NIMDeleteCachedFileCallback,
      json_extension: string): void {
