...
export interface NIMTeamInfo {
  ...
  update_info_mode: NIMTeamUpdateInfoMode;/** < int, 谁可以修改群资料，属性本身只有群主管理员可以修改,
                        默认kNIMTeamUpdateInfoModeManager(0)/kNIMTeamUpdateInfoModeEveryone(1) */
  update_custom_mode: NIMTeamUpdateCustomMode;/** < int, 谁可以更新群自定义属性，属性本身只有群主管理员可以修改,
                        默认kNIMTeamUpdateCustomModeManager(0)/kNIMTeamUpdateCustomModeEveryone(1) */
...
export interface NIMTeamAPI {
  ...
  GetTeamInfoBatchSFTrans(cb: NIMGetTeamInfoBatchSFTransCallback, time_tag: number, json_extension: string): void;
