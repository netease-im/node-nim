#ifndef NIM_NODE_ASYNC_QUEUE_H
#define NIM_NODE_ASYNC_QUEUE_H
...
public:
    ...
    {
        auto ret = ::uv_async_init(loop, h_, async_callback);
        h_->data = this;
    }
    ...
    void close()
    {
        if (!empty())
        {
            // printf(" You should close this queue after taking all the elements!");
        }
        closed_ = true;
    }
    bool closed() const
