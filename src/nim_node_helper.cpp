...
napi_status nim_napi_get_value_utf8string_list(Isolate* isolate, const Local<Value>& value, std::list<utf8_string>& out) {
    ...
    for (size_t i = 0; i < values->Length(); i++) {
        UTF8String str;
        nim_napi_get_value_utf8string(isolate, values->Get(isolate->GetCurrentContext(), i).ToLocalChecked(), str);
...
napi_status nim_napi_get_value_utf8string(Isolate* isolate, const Local<Value>& value, UTF8String& out) {
    ...
    do {
        ...
        if (!str)
            break;
#if NODE_MAJOR_VERSION <= 10
        len = value.As<String>()->WriteUtf8(str, len, nullptr, String::REPLACE_INVALID_UTF8 | String::NO_NULL_TERMINATION);
#else
...
napi_status nim_napi_get_value_uint32_list(Isolate* isolate, const Local<Value>& value, std::list<uint32_t>& out) {
    ...
    for (size_t i = 0; i < values->Length(); i++) {
        uint32_t temp;
        nim_napi_get_value_uint32(isolate, values->Get(isolate->GetCurrentContext(), i).ToLocalChecked(), temp);
...
napi_status nim_napi_get_object_value_utf8string_list(Isolate* isolate,
    ...
    Local<Value> value;
    if (get_object_value(isolate, obj, keyName, value) == napi_ok)
        return nim_napi_get_value_utf8string_list(isolate, value, out);
    return napi_invalid_arg;
}
...
napi_status nim_napi_get_object_value_uint32_list(Isolate* isolate, const Local<Object>& obj, const utf8_string& keyName, std::list<uint32_t>& out) {
    Local<Value> value;
    if (get_object_value(isolate, obj, keyName, value) == napi_ok)
        return nim_napi_get_value_uint32_list(isolate, value, out);
    return napi_invalid_arg;
}
...
napi_status nim_napi_set_object_value_utf8string_list(Isolate* isolate,
    ...
    Local<Value> key = nim_napi_new_utf8string(isolate, keyName.c_str());
    Local<Value> value = nim_napi_new_utf8string_list(isolate, keyValue);
    obj->Set(isolate->GetCurrentContext(), key, value);
    return napi_ok;
}
...
Local<Value> nim_napi_new_utf8string_list(Isolate* isolate, const std::list<utf8_string>& keyValue) {
    Local<Array> array = Array::New(isolate, keyValue.size());
    uint32_t index = 0;
    for (auto&& v : keyValue) {
        Local<String> obj = String::NewFromUtf8(isolate, v.c_str(), NewStringType::kInternalized).ToLocalChecked();
