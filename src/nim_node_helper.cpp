...
napi_status nim_napi_get_value_utf8string_list(Isolate* isolate,
    ...
    for (size_t i = 0; i < values->Length(); i++) {
        UTF8String str;
        nim_napi_get_value_utf8string(
            isolate,
            values->Get(isolate->GetCurrentContext(), i).ToLocalChecked(), str);
        out.push_back(str.toUtf8String());
    }
    return status;
}

napi_status nim_napi_get_value_utf8string(Isolate* isolate,
                                          const Local<Value>& value,
                                          UTF8String& out) {
    if (!value->IsString())
        return napi_invalid_arg;
    ...
    do {
        ...
        int len = value.As<String>()->Utf8Length();
#else
...
napi_status nim_napi_get_value_uint32(Isolate* isolate,
                                      const Local<Value>& value,
                                      uint32_t& out) {
    if (!value->IsUint32())
        return napi_invalid_arg;
...
napi_status nim_napi_get_value_int32(Isolate* isolate,
                                     const Local<Value>& value,
                                     int32_t& out) {
    if (!value->IsInt32())
        return napi_invalid_arg;
...
napi_status nim_napi_get_value_bool(Isolate* isolate,
                                    const Local<Value>& value,
                                    bool& out) {
    if (!value->IsBoolean())
        return napi_invalid_arg;
...
napi_status nim_napi_get_value_int64(Isolate* isolate,
                                     const Local<Value>& value,
                                     int64_t& out) {
    if (!value->IsNumber())
        return napi_invalid_arg;
...
napi_status nim_napi_get_value_uint64(Isolate* isolate,
                                      const Local<Value>& value,
                                      uint64_t& out) {
    if (!value->IsNumber())
        return napi_invalid_arg;
...
static napi_status get_object_value(Isolate* isolate,
...
napi_status nim_napi_get_object_value(Isolate* isolate,
                                      const Local<Object>& obj,
                                      const utf8_string& keyName,
                                      Local<Value>& out) {
    if (get_object_value(isolate, obj, keyName, out) == napi_ok &&
        !out.IsEmpty())
        return napi_ok;
    return napi_invalid_arg;
}

napi_status nim_napi_get_object_value_utf8string(Isolate* isolate,
                                                 const Local<Object>& obj,
                                                 const utf8_string& keyName,
                                                 UTF8String& out) {
    Local<Value> value;
...
napi_status nim_napi_get_object_value_uint32(Isolate* isolate,
                                             const Local<Object>& obj,
                                             const utf8_string& keyName,
                                             uint32_t& out) {
    Local<Value> value;
...
napi_status nim_napi_get_object_value_int32(Isolate* isolate,
                                            const Local<Object>& obj,
                                            const utf8_string& keyName,
                                            int32_t& out) {
    Local<Value> value;
...
napi_status nim_napi_get_object_value_bool(Isolate* isolate,
                                           const Local<Object>& obj,
                                           const utf8_string& keyName,
                                           bool& out) {
    Local<Value> value;
...
napi_status nim_napi_get_object_value_int64(Isolate* isolate,
                                            const Local<Object>& obj,
                                            const utf8_string& keyName,
                                            int64_t& out) {
    Local<Value> value;
...
napi_status nim_napi_get_object_value_uint64(Isolate* isolate,
                                             const Local<Object>& obj,
                                             const utf8_string& keyName,
                                             uint64_t& out) {
    Local<Value> value;
...
napi_status nim_napi_set_object_value_utf8string(Isolate* isolate,
                                                 Local<Object>& obj,
                                                 const utf8_string& keyName,
                                                 const utf8_string& keyValue) {
...
napi_status nim_napi_set_object_value_uint32(Isolate* isolate,
                                             Local<Object>& obj,
                                             const utf8_string& keyName,
...
napi_status nim_napi_set_object_value_int32(Isolate* isolate,
                                            Local<Object>& obj,
                                            const utf8_string& keyName,
...
napi_status nim_napi_set_object_value_bool(Isolate* isolate,
                                           Local<Object>& obj,
                                           const utf8_string& keyName,
                                           const bool& keyValue) {
    Local<Value> key = nim_napi_new_utf8string(isolate, keyName.c_str());
...
napi_status nim_napi_set_object_value_int64(Isolate* isolate,
                                            Local<Object>& obj,
                                            const utf8_string& keyName,
                                            const int64_t& keyValue) {
    Local<Value> key = nim_napi_new_utf8string(isolate, keyName.c_str());
...
napi_status nim_napi_set_object_value_uint64(Isolate* isolate,
                                             Local<Object>& obj,
                                             const utf8_string& keyName,
                                             const uint64_t& keyValue) {
    Local<Value> key = nim_napi_new_utf8string(isolate, keyName.c_str());
...
Local<Value> nim_napi_new_utf8string(Isolate* isolate,
                                     const utf8_char* keyValue) {
    return String::NewFromUtf8(isolate, keyValue ? keyValue : "",
...
napi_status nim_napi_assemble_string_array(Isolate* isolate,
                                           const std::list<utf8_string>& strs,
                                           Local<Array>& arr) {
    int index = 0;
    for (auto const& str : strs) {
        arr->Set(isolate->GetCurrentContext(), index++,
                 nim_napi_new_utf8string(isolate, str.c_str()));
    }
    return napi_ok;
}

napi_status nim_napi_assemble_string_map(
    Isolate* isolate,
    const std::map<utf8_string, utf8_string>& ipt,
    Local<Object>& opt) {
    for (auto const& i : ipt) {
        opt->Set(isolate->GetCurrentContext(),
                 nim_napi_new_utf8string(isolate, i.first.c_str()),
                 nim_napi_new_utf8string(isolate, i.second.c_str()));
    }
    return napi_ok;
}

napi_status nim_napi_UTF8Strings_to_utf8strings(
    const std::list<UTF8String>& strs,
    std::list<utf8_string>& out) {
    for (auto&& i : strs) {
