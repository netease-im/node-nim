...
class JsonDecoder : public XDecoder<JsonDecoder>, private noncopyable {
    ...
    public:
        MemberIterator(const rapidjson::Value::ConstMemberIterator iter, JsonDecoder* parent)
            : _iter(iter)
            , _parent(parent) {}
        bool operator!=(const MemberIterator& that) const { return _iter != that._iter; }
        MemberIterator& operator++() {
            ++_iter;
            return *this;
        }
        const char* Key() const { return _iter->name.GetString(); }
        JsonDecoder& Val() const { return _parent->member(*this, *(_parent->alloc())); }
...
public:
    ...
    JsonDecoder(const std::string& str, bool isfile = false)
        : xdoc_type(NULL, "")
    ...
    JsonDecoder(const rapidjson::Value* v)
        : xdoc_type(NULL, "")
        , _doc(NULL)
        , _val(v) {}
    ...
    inline const char* Type() const { return "json"; }

public:
// decode
#define XPACK_JSON_DECODE(nullVal, f, stoifunc)           \
    bool isNull;                                          \
    ...
    bool decode(const char* key, char& val, const Extend* ext) { XPACK_JSON_DECODE('\0', GetInt, std::stoi); }
    bool decode(const char* key, signed char& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt, std::stoi); }
    bool decode(const char* key, unsigned char& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt, std::stoi); }
    bool decode(const char* key, short& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt, std::stoi); }
    bool decode(const char* key, unsigned short& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt, std::stoi); }
    bool decode(const char* key, int& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt, std::stoi); }
    bool decode(const char* key, unsigned int& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetUint, std::stoul); }
    bool decode(const char* key, long& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt64, std::stoll); }
    bool decode(const char* key, unsigned long& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetUint64, std::stoull); }
    bool decode(const char* key, long long& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetInt64, std::stoll); }
    bool decode(const char* key, unsigned long long& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetUint64, std::stoull); }
    bool decode(const char* key, float& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetFloat, std::stof); }
    bool decode(const char* key, double& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetDouble, std::stod); }
    bool decode(const char* key, long double& val, const Extend* ext) { XPACK_JSON_DECODE(0, GetDouble, std::stod); }
    ...
    // iter
    Iterator Begin() { return Iterator(_val->MemberBegin(), this); }
    Iterator End() { return Iterator(_val->MemberEnd(), this); }
    operator bool() const { return NULL != _val; }
