...
void SessionEventHandler::OnDeleteSessionRoamingMessageCallback(const BaseCallbackPtr& bcb,
                                                                nim::NIMResCode res_code,
                                                                nim::NIMSessionType to_type,
                                                                const std::string& session_id) {
    node_async_call::async_call([=]() {
        SessionEventHandler::GetInstance()->Node_OnDeleteSessionRoamingMessageCallback(bcb, res_code, to_type, session_id);
    });
}

void SessionEventHandler::OnSetMultiUnreadCountZeroCallback(const BaseCallbackPtr& bcb,
                                                            nim::NIMResCode res_code,
                                                            const std::list<nim::SessionData>& session_data_list,
                                                            int unread_count) {
    node_async_call::async_call([=]() {
        SessionEventHandler::GetInstance()->Node_OnSetMultiUnreadCountZeroCallback(bcb, res_code, session_data_list, unread_count);
    });
}
...
void SessionEventHandler::Node_OnDeleteSessionRoamingMessageCallback(const BaseCallbackPtr& bcb,
                                                                     nim::NIMResCode res_code,
                                                                     nim::NIMSessionType to_type,
                                                                     const std::string& session_id) {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    const unsigned argc = 3;
    Local<Value> argv[argc] = {nim_napi_new_uint32(isolate, (uint32_t)(res_code)), nim_napi_new_uint32(isolate, (uint32_t)(to_type)),
                               nim_napi_new_utf8string(isolate, session_id.c_str())};
    bcb->callback_.Get(isolate)->Call(isolate->GetCurrentContext(), bcb->data_.Get(isolate), argc, argv);
}

void SessionEventHandler::Node_OnSetMultiUnreadCountZeroCallback(const BaseCallbackPtr& bcb,
                                                                 nim::NIMResCode res_code,
                                                                 const std::list<nim::SessionData>& session_data_list,
                                                                 int unread_count) {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    const unsigned argc = 3;
    Local<Array> session_list_array = Array::New(isolate, session_data_list.size());
    uint32_t index = 0;
    for (auto& session : session_data_list) {
        auto str = nim_napi_new_utf8string(isolate, nim::SessionData::ToJsonString(session).c_str());
        auto result = v8::JSON::Parse(isolate->GetCurrentContext(), str->ToString(isolate->GetCurrentContext()).ToLocalChecked());
        session_list_array->Set(isolate->GetCurrentContext(), index++, result.ToLocalChecked());
    }
    Local<Value> argv[argc] = {nim_napi_new_int32(isolate, res_code), session_list_array, nim_napi_new_int32(isolate, unread_count)};
    bcb->callback_.Get(isolate)->Call(isolate->GetCurrentContext(), bcb->data_.Get(isolate), argc, argv);
}
