...
public:
    ...
    static void OnLoginCallback(const BaseCallbackPtr& bcb,
                                bool relogin_cb,
                                const nim::LoginRes& login_res);
    static void OnFunctionCallback();
    static void OnLogoutCallback(const BaseCallbackPtr& bcb,
    ...
    static void OnSyncMultiportPushConfigCallback(const BaseCallbackPtr& bcb,
                                                  int rescode,
                                                  bool open);
    static void OnGetServerCurrentTimeCallback(const BaseCallbackPtr& bcb,
                                               int rescode,
                                               bool calc_local,
                                               uint64_t time);
    static void OnReloginRequestTokenCb(const BaseCallbackPtr& bcb,
                                        std::string* res);

private:
    void Node_OnLoginCallback(const BaseCallbackPtr& bcb,
                              bool relogin_cb,
                              const nim::LoginRes& login_res);
    void Node_OnFunctionCallback();
    void Node_OnLogoutCallback(const BaseCallbackPtr& bcb,
                               nim::NIMResCode rescode);
    ...
    void Node_OnSyncMultiportPushConfigCallback(const BaseCallbackPtr& bcb,
                                                int rescode,
                                                bool open);
    void Node_OnGetServerCurrentTimeCallback(const BaseCallbackPtr& bcb,
                                             int rescode,
                                             bool calc_local,
                                             uint64_t time);
    void Node_OnReloginRequestTokenCb(const BaseCallbackPtr& bcb,
                                      std::string res);
};
