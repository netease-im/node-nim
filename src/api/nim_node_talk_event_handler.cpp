...
void TalkEventHandler::OnRecallMsgsCallback(bool active, const nim::NIMResCode res, const std::list<nim::RecallMsgNotify>& msgs)
{
    node_async_call::async_call([=]() {
        TalkEventHandler::GetInstance()->Node_OnRecallMsgsCallback(active, res, msgs);
    });
}
...
void TalkEventHandler::Node_OnRecallMsgsCallback(bool active, const nim::NIMResCode res, const std::list<nim::RecallMsgNotify>& msgs)
{
    Isolate *isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    ...
    Local<Value> argv[argc] = { nim_napi_new_bool(isolate, active),
            nim_napi_new_uint32(isolate, (uint32_t)res),
            obj 
    };
    auto it = callbacks_.find("OnRecallMsgsCallback");
    if (it != callbacks_.end())
    {
        it->second->callback_.Get(isolate)->Call(isolate->GetCurrentContext(), it->second->data_.Get(isolate), argc, argv);
    }   
