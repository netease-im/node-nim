...
void GlobalEventHandler::OnDetectProxyCallback(const BaseCallbackPtr &bcb, bool connect, nim::NIMProxyDetectStep step, const utf8_string &json_extention)
{
    node_async_call::async_call([=]() {
...
void GlobalEventHandler::Node_OnDetectProxyCallback(const BaseCallbackPtr &bcb, bool connect, nim::NIMProxyDetectStep step, const utf8_string& json_extention)
{
    Isolate *isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    const unsigned argc = 3;
    Local<Value> argv[argc] = { nim_napi_new_bool(isolate, connect), 
                nim_napi_new_uint32(isolate, (uint32_t)step),
...
void GlobalEventHandler::Node_OnGetCachedFileInfoCallback(const BaseCallbackPtr &bcb, nim::NIMResCode rescode, const nim::Global::CachedFileInfo &info)
{
    Isolate *isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
    const unsigned argc = 2;
