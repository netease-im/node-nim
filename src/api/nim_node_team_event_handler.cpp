...
namespace nim_node {
void TeamEventHandler::OnTeamEventCallback(const BaseCallbackPtr& bcb,
                                           const nim::TeamEvent& team_event) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnTeamEventCallback(bcb,
                                                                  team_event);
    });
}

void TeamEventHandler::OnQueryTeamMembersInvitorCallback(
    const BaseCallbackPtr& bcb,
    nim::NIMResCode error_code,
    const utf8_string& tid,
    const std::map<utf8_string, utf8_string>& invitor_map) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryTeamMembersInvitorCallback(
            bcb, error_code, tid, invitor_map);
    });
}

void TeamEventHandler::OnQueryMyTeamsCallback(
    const BaseCallbackPtr& bcb,
    int team_count,
    const std::list<utf8_string>& team_id_list) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryMyTeamsCallback(
            bcb, team_count, team_id_list);
    });
}

void TeamEventHandler::OnQueryTeamMembersCallback(
    const BaseCallbackPtr& bcb,
    const utf8_string& tid,
    int member_count,
    const std::list<nim::TeamMemberProperty>& props) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryTeamMembersCallback(
            bcb, tid, member_count, props);
    });
}

void TeamEventHandler::OnQueryTeamMemberCallback(
    const BaseCallbackPtr& bcb,
    const nim::TeamMemberProperty& team_member_property) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryTeamMemberCallback(
            bcb, team_member_property);
    });
}

void TeamEventHandler::OnQueryAllMyTeamsInfoCallback(
    const BaseCallbackPtr& bcb,
    int team_count,
    const std::list<nim::TeamInfo>& team_info_list) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryAllMyTeamsInfoCallback(
            bcb, team_count, team_info_list);
    });
}

void TeamEventHandler::OnUpdateTInfoLocalCallback(
    const BaseCallbackPtr& bcb,
    const std::list<std::string>& success_ids,
    const std::list<std::string>& failure_ids) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnUpdateTInfoLocalCallback(
            bcb, success_ids, failure_ids);
    });
}

void TeamEventHandler::OnQueryMyAllMemberInfosCallback(
    const BaseCallbackPtr& bcb,
    int count,
    const std::list<nim::TeamMemberProperty>& all_my_member_info_list) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryMyAllMemberInfosCallback(
            bcb, count, all_my_member_info_list);
    });
}

void TeamEventHandler::OnQueryTeamInfoCallback(const BaseCallbackPtr& bcb,
                                               const utf8_string& tid,
                                               const nim::TeamInfo& result) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryTeamInfoCallback(bcb, tid,
                                                                      result);
    });
}

void TeamEventHandler::OnQueryMembersOnlineCallback(
    const BaseCallbackPtr& bcb,
    nim::NIMResCode error_code,
    const utf8_string& tid,
    const std::list<nim::TeamMemberProperty>& team_member_propertys) {
    node_async_call::async_call([=]() {
        TeamEventHandler::GetInstance()->Node_OnQueryMembersOnlineCallback(
            bcb, error_code, tid, team_member_propertys);
    });
}

void TeamEventHandler::Node_OnTeamEventCallback(
    const BaseCallbackPtr& bcb,
    const nim::TeamEvent& team_event) {
...
void TeamEventHandler::Node_OnQueryTeamMembersInvitorCallback(
    const BaseCallbackPtr& bcb,
    nim::NIMResCode error_code,
    const utf8_string& tid,
    const std::map<utf8_string, utf8_string>& invitor_map) {
...
void TeamEventHandler::Node_OnQueryMyTeamsCallback(
    const BaseCallbackPtr& bcb,
    int team_count,
    const std::list<utf8_string>& team_id_list) {
    Isolate* isolate = Isolate::GetCurrent();
...
void TeamEventHandler::Node_OnQueryTeamMembersCallback(
    const BaseCallbackPtr& bcb,
    const utf8_string& tid,
    int member_count,
    const std::list<nim::TeamMemberProperty>& props) {
...
void TeamEventHandler::Node_OnQueryTeamMemberCallback(
    const BaseCallbackPtr& bcb,
    const nim::TeamMemberProperty& team_member_property) {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
...
void TeamEventHandler::Node_OnQueryAllMyTeamsInfoCallback(
    const BaseCallbackPtr& bcb,
    int team_count,
    const std::list<nim::TeamInfo>& team_info_list) {
    Isolate* isolate = Isolate::GetCurrent();
...
void TeamEventHandler::Node_OnUpdateTInfoLocalCallback(
    const BaseCallbackPtr& bcb,
    const std::list<std::string>& success_ids,
    const std::list<std::string>& failure_ids) {
    Isolate* isolate = Isolate::GetCurrent();
...
void TeamEventHandler::Node_OnQueryMyAllMemberInfosCallback(
    const BaseCallbackPtr& bcb,
    int count,
    const std::list<nim::TeamMemberProperty>& all_my_member_info_list) {
    Isolate* isolate = Isolate::GetCurrent();
...
void TeamEventHandler::Node_OnQueryTeamInfoCallback(
    const BaseCallbackPtr& bcb,
    const utf8_string& tid,
    const nim::TeamInfo& result) {
    Isolate* isolate = Isolate::GetCurrent();
...
void TeamEventHandler::Node_OnQueryMembersOnlineCallback(
    const BaseCallbackPtr& bcb,
    nim::NIMResCode error_code,
    const utf8_string& tid,
    const std::list<nim::TeamMemberProperty>& team_member_propertys) {
