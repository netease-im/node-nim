...
namespace nim_node {
void SessionOnlineEventHandler::OnQuerySessionInfoCallback(const BaseCallbackPtr& bcb,
                                                           nim::NIMResCode rescode,
                                                           const nim::SessionOnLineServiceHelper::SessionInfo& info) {
    node_async_call::async_call([=]() {
        SessionOnlineEventHandler::GetInstance()->Node_OnQuerySessionInfoCallback(bcb, rescode, info);
    });
}
void SessionOnlineEventHandler::OnQuerySessionListCallabck(const BaseCallbackPtr& bcb,
                                                           const nim::SessionOnLineServiceHelper::QuerySessionListResult& info) {
    node_async_call::async_call([=]() {
...
void SessionOnlineEventHandler::Node_OnQuerySessionInfoCallback(const BaseCallbackPtr& bcb,
                                                                nim::NIMResCode rescode,
                                                                const nim::SessionOnLineServiceHelper::SessionInfo& info) {
    ...
    Local<Value> argv[argc] = {nim_napi_new_uint32(isolate, (uint32_t)(rescode)), res};
    bcb->callback_.Get(isolate)->Call(isolate->GetCurrentContext(), bcb->data_.Get(isolate), argc, argv);
}
void SessionOnlineEventHandler::Node_OnQuerySessionListCallabck(const BaseCallbackPtr& bcb,
                                                                const nim::SessionOnLineServiceHelper::QuerySessionListResult& info) {
    Isolate* isolate = Isolate::GetCurrent();
    HandleScope scope(isolate);
