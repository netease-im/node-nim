...
describe('********************TalkEx********************', function () {
  ...
  describe('#updateCollectExt', function () {
    it('updateCollectExt should return 200', function (done) {
      talkex.updateCollectExt({
        create_time: collect_info.create_time,
        id: collect_info.id
      }, 'test', function (res_code, info) {
        assert.strictEqual(res_code, 200)
        done()
      })
    })
  ...
  describe('#removeCollects', function () {
    it('removeCollects should return 200', function (done) {
      talkex.removeCollects({
        list: [{
          create_time: collect_info.create_time,
          id: collect_info.id
        }]
      }, function (res_code, count) {
        assert.strictEqual(res_code, 200)
        done()
      })
    })
  ...
  describe('#addQuickComment', function () {
    it('addQuickComment should return 200', function (done) {
      talkex.addQuickComment(
        immessage,
        {
          id: `${time}`,
          from_account: test_info.mainUser,
          reply_type: 1,
          client_id: immessage.client_msg_id,
          server_id: immessage.server_msg_id,
          time: time,
          ext: 'test',
          need_push: 0,
          need_badge: 0,
  ...
  describe('#addPinMessage', function () {
    it('addPinMessage should return 200', function (done) {
      talkex.addPinMessage(
        immessage,
        {
          id: new Date().getTime().toString(),
          session_id: test_info.mainUser,
          to_account: test_info.mainUser,
          client_id: immessage.client_msg_id,
          to_type: 0,
          message_time: new Date().getTime(),
          operator_account: test_info.mainUser,
          ext: 'test',
  ...
  describe('#updatePinMessage', function () {
    it('updatePinMessage should return 200', function (done) {
      talkex.updatePinMessage({
        session: test_info.mainUser,
        to_type: 0,
        id: pin_id,
        ext: 'node test'
      }, function (res_code, session, to_type, info) {
